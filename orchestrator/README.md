...
It's also the **only service connected to MQTT, PostgREST, and the Gemini API** (aside from `telegram-adapter`/`web-adapter`, which own their own MQTT connections as channel adapters, and `doc-ingestion-worker`/`image-generation-worker`'s narrow Gemini exceptions — see the root [README's design notes](../README.md#design-notes)). None of the three workers has a direct connection to any of those systems; all three reach `orchestrator` through the small internal HTTP API described below instead.
...
Four things can pause a turn instead of running inline:
...
- **`generate_image`** (a real photo via `image-generation-worker`) — if the model passed a `device_id` and a photo is already saved for that device, this resolves immediately with no worker round trip at all; otherwise it pauses the same way as `generate_document` while the worker tries a real image search and falls back to Gemini generation.
...
**Whatever's sent to/from Gemini and ends up associated with a device (or isn't tied to any one) is saved automatically to `home.device_document`**, not just when the model remembers to call `attach_document`: the photo/document behind an approved `create_device`/`update_device` (`_save_onboarding_attachments`), and a successful `generate_document`/`generate_image` result (`_save_generated_attachment`) — against the `device_id` the model passed, or as a general household document (`device_id=NULL`) if it didn't. Both are best-effort — a save failure is logged, never blocks the device write or file delivery.
...
| `POST /internal/doc-generation/result` | `doc-generation-worker` | Delivers the result of a `/generate` request once rendering finishes (success + base64 file bytes, or failure + error) — resumes the paused turn, saves the file, and attaches it to the reply |
| `POST /internal/image/result` | `image-generation-worker` | Delivers the result of a `/generate-image` request (success + base64 JPEG + which path produced it, or failure + error) — resumes the paused turn, saves the image, and attaches it to the reply |
...
| `list_house_documents` | auto | General household documents/images saved with no specific device (`device_id IS NULL`) — the read side of the auto-save behavior above |
| `attach_document` | auto | Attaches a manual/photo/note to an existing device, or to the household in general if `device_id` is omitted (`home.device_document`) — purely additive, nothing destroyed |
...
| `generate_image` | n/a (async) | Gets a real picture (reuses one on file, else searches, else generates) and sends it back as a Telegram photo |
