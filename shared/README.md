...
| Module | Purpose |
|---|---|
| `message.py` | The normalized MQTT message contract every channel adapter speaks (`NormalizedMessage`), including `Attachment`/`AttachmentKind` (image/document/audio — a message is text plus zero-or-more attachments, not "one type"; `MessageType` is now just `TEXT`/`COMMAND`/reserved `TELEMETRY`), topic naming helpers (`inbound_topic`, `outbound_topic`), and the JSON body schemas used between `orchestrator` and `doc-ingestion-worker` over HTTP (`DocIngestionRequest`/`DocIngestionResult`) — these used to be MQTT event payloads, now they're internal API request/response bodies. |
...
| `claude.py` | Two things: `call_structured()` (unchanged — forces Claude to return a Pydantic-validated object via `tool_choice`, retrying on schema mismatch; used by `doc-ingestion-worker`'s vision extraction) and `run_agent_loop()`/`resume_agent_loop()` (new — the multi-turn tool-use loop `orchestrator` runs on every message: executes same-turn tool calls concurrently and combines their results into one message, sets `cache_control` breakpoints on the system prompt and the growing history, and supports pausing/resuming for the one asynchronous tool). Also `image_block()`/`build_content_blocks()` — turn an `Attachment` into the content blocks Claude's API expects (a `document`-kind attachment that's still a URL gets downloaded and base64-encoded, since that content-block type has no `url` source unlike `image`). **Async throughout** (`AsyncAnthropic`, not the sync `Anthropic` client): now that all conversational Claude calls are centralized in `orchestrator`, a blocking synchronous call would stall every conversation in the house, not just one process's worth of work. |
