...
- **Channel adapters** (`telegram-adapter`, `web-adapter`) are the one deliberate exception on the inbound/outbound side: each manages its own Telegram/WebSocket connection *and* its own MQTT connection, publishing/subscribing directly to `home/inbound/<channel>/*` and `home/outbound/<channel>/*`. They're dumb translators — they turn whatever the channel gives them into a normalized message and publish it, and turn outbound normalized messages (including approval-prompt buttons) back into channel-native replies. The `orchestrator` never knows or cares which channel a message came from.
...
├── Dockerfile.service      # one generic Dockerfile for all 4 services, parameterized via build args
├── build-and-push-images.sh # builds + tags + pushes all 4 images to Docker Hub — see below
...
`build-and-push-images.sh` (repo root) builds from `Dockerfile.service` — same one `docker-compose-local.yml` uses — and pushes to the 4 repositories above. Log in once per machine/session (always interactive, never scripted):

```bash
docker login
```

**All 4 at once** (the common case — e.g. after a change to `shared/`, which every service depends on):
...
Valid names: `telegram-adapter`, `web-adapter`, `orchestrator`, `doc-ingestion-worker` (an unknown name fails fast with that list, instead of silently building nothing). Combine with either mode:

```bash
./build-and-push-images.sh --build-only                    # build all 4, push none — sanity-check first
