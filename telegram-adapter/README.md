...
- **Albums (several photos/documents sent together) become one message, not several.** Telegram delivers each item of an album as its own separate Update sharing a `media_group_id` — this adapter buffers them and, once ~1.5s pass with no new item for that group, publishes a single `NormalizedMessage` with every attachment from the album, using whichever item carried the caption. `orchestrator`/Gemini then works through the attachments one at a time (see the root README's design notes).
- Shows Telegram's native "escribiendo..." (typing) indicator for as long as a turn is in flight — started right after publishing an inbound message, stopped as soon as any reply for that chat comes back on `home/outbound/telegram/<chat_id>` (`aiogram.utils.chat_action.ChatActionSender`).
- Each `NormalizedMessage` arriving on `home/outbound/telegram/<chat_id>` gets sent back to that chat as a plain text message — with an inline keyboard attached if the message carries `actions` (e.g. an Aprobar/Rechazar approval prompt from `orchestrator`'s Human-in-the-Loop gate). If it carries `attachments` (e.g. a file from the `generate_document` tool), each one is decoded (from a `data:` base64 URI, or downloaded if it's a plain URL) and sent as a real Telegram document via `bot.send_document`.
